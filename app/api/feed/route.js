...
const parser = new Parser({
  timeout: 9000,
  headers: {"User-Agent": "Meanwhile/2.0"},
...
async function enrichStoryImage(item) {
  ...
  try {
    const response = await fetch(item.url, {redirect:"follow", headers:{"User-Agent":"Mozilla/5.0 Meanwhile/5.0"}, signal:AbortSignal.timeout(2800)});
...
async function loadVisualShelf(identity, count = 80) {
  ...
  await Promise.all(searches.map(async search => {
    try {
      ...
      const response = await fetch(`https://commons.wikimedia.org/w/api.php?${params}`, {headers:{"User-Agent":"Meanwhile/10.0 (visual shelf; attributed Commons media)"}, signal:AbortSignal.timeout(5500)});
...
async function sharedVideoSources() {
  ...
  try { const response = await fetch(url, {method:"POST", headers:{authorization:`Bearer ${token}`,"content-type":"application/json"}, body:JSON.stringify(["GET","meanwhile:sources"]), cache:"no-store"}); const value = (await response.json()).result; return value ? JSON.parse(value) : fallback; } catch { return fallback; }
}
