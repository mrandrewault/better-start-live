...
export default function Home() {
  ...
  return <main className="shell">
    ...
    <section className="gallerySection"><div className="sectionHead wallHead"><div><span>Your coffee table</span><h2>Good Stuff</h2></div><p>Chosen for joy, curiosity & variety</p></div>{visibleBatches.length ? visibleBatches.map((batch, batchIndex) => <div className="gallery galleryBatch" key={batchIndex}>{batch.map((item, index) => <Story item={item} index={batchIndex * BATCH_SIZE + index} onRate={rate} key={item.canonicalUrl} />)}</div>) : <div className="loading"><span>Composing today&apos;s wall</span><i /><i /><i /></div>}
