...
export default function Home() {
  ...
  const share = async item => { const text = `I found this on Better Start — rage-free news, information and good times.\n\n${item.title}`; try { if (navigator.share) await navigator.share({title: item.title, text, url: item.url}); else { await navigator.clipboard.writeText(`${text}\n${item.url}`); setEditionNote("Share message copied"); } } catch {} };
